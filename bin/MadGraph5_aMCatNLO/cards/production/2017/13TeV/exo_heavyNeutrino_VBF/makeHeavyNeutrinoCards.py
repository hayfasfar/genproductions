...
def makeHeavyNeutrinoCards(mass, coupling, flavours):
  baseName = 'HeavyNeutrino_VBF_trilepton_M-' + str(mass) + '_V-' + str(coupling) + '_' + flavours + '_NLO'

  try:    os.makedirs(baseName)
  except: pass

  for card in ['madspin_card', 'extramodels', 'run_card', 'proc_card', 'customizecards']:
    shutil.copyfile('templateCards/HeavyNeutrino_VBF_trilepton_template_NLO_' + card + '.dat', baseName + '/' + baseName + '_' + card + '.dat')
